...
synchronized是怎样实现锁的呢？它的底层实现是什么呢？

首先我们先来了解一个重要的概念，Monitor。
...
同步方法是通过一个`ACC_SYNCHRONIZED`标志来确定。
...
<div class='tip'> <p>
    当字节码解释器执行了monitorenter轻度锁住一个对象后，就会在线程的栈上显示或隐式分配一个LockRecord（锁记录），该锁记录存储着对象头中的MarkWord的拷贝，里面Owner指向对象的markword，并将对象的markword的锁记录指针指向获得锁线程的LockRecord。
</p></div>
...
在JDK1.6之前，synchronized只是一个重量级的锁，当JDK1.6进行更新之后，引入了偏向锁和轻量级锁的概念，所以锁的状态变成了以上的四种，现在我们就来研究一下这四种锁的状态。
