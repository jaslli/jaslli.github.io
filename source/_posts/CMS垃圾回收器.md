...
这个阶段相当于提前做一些下个阶段带来的工作，尽可能的减少`STW`，还会处理一些Dirty的对象，这个阶段是并发的。

这个阶段默认持续的时间是5秒，即时间超过5秒，就会终止这个阶段，或者当eden区使用内存值小于CMSScheduleRemarkEdenPenetration，默认50%时，也会退出这个阶段。

可终止的预处理阶段就是为了减少重新标记阶段的时间，当在可终止的预处理阶段时碰上了`yound GC`，因为重新标记是扫描整个堆的，所以碰上了`yound GC`就会大大减少下个阶段的`STW`。
...
因为这个阶段是并发的，有可能也会产生垃圾，这次GC的标记阶段已经过去了，所以就只能下次GC的时候清理了。

因为是标记-清除算法，所以这个阶段就会产生一些内存碎片。
...
虽然CMS有扫描到年轻代，但是CMS是处理老年代的死亡对象的，所以注意和Full GC进行区分。

## CMS回收过程会发生STW

尽管CMS是并发回收，但是也涉及到可达性分析算法，所以一定会发生STW，只是CMS尽量减短了STW的时间。
...
因为CMS是并发回收的，所以用户线程也是会可能在GC的过程中消耗内存，年轻代会因为一些原因就将一些存活对象放入老年代,将部分年轻代存活的对象放入老年代，而此时老年代没有足够的空间，此时就会出现`Concurrent Mode Failure`。
...
- -XX:+CMSScavengeBeforeRemark
  ...
  在执行remark操作之前先做一次young GC，因为重新标记是扫描整个堆的，young GC会帮助扫描年轻代，减少重新标记的STW时间。
